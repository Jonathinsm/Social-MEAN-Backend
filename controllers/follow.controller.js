...
//Save un follow
followCtrl.deleteFollow = (req, res) => {
...
//Save un follow
followCtrl.getFollowingUsers = (req,res)=>{
    var userId = req.user.sub;
    ...
    .populate('followed','_id name surname nick email image')
...
followCtrl.getFollowedUsers = (req,res)=>{
    var userId = req.user.sub;
    if(req.params.id && req.params.page){
        userId = req.params.id;
    };
    ...
    .populate('user','_id name surname nick email image')
    .paginate(page, itemsPerPage, (err, follows, total)=>{
        if(err) return res.status(500).send({message: 'Error in the server'});
...
followCtrl.getMyFollows = (req,res)=>{
    var userId = req.user.sub;
