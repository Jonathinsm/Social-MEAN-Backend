...
userCtrl.getUsers = (req,res)=>{
 ...
 //Return a list of following and followed users
...
//Get a count total of following and followed users
userCtrl.getCounters = (req,res)=>{
...
//Return an recount of following, followed users and publications
...
userCtrl.uploadImage = (req,res)=>{
    ...
    if(req.files){
        ...
        if(userId != req.user.sub){
            ...
            return removeFilesOfUploads(res, file_path, 'You dont haver permissions to update image user');
        };
        if(file_ext == 'png' || file_ext == 'jpg' || file_ext == 'jpeg' || file_ext == 'gif'){
            ...
            //Return an error if file extansion is invalid
            return removeFilesOfUploads(res, file_path, 'Invalid extension');
        }
    }else{
        //Return an error if the file cant load
        return res.status(200).send({message: 'Files has not send'});
    };
};

//Interrup uploading file
userCtrl.removeFilesOfUploads = (res, file_path, message)=>{
    fs.unlink(file_path, (err) => {
        return res.status(200).send({message: message});
    });
};

//Get the file for show
